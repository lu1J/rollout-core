...
if ($VerifyOutage) {
    ...
    $cached = Invoke-Json 'GET' "$DemoUrl/demo/payment?userId=day5-new" $null
    ...
    Write-Output 'REAL_DAY5_OUTAGE_E2E=PASSED'
    return
}
...
$null = Invoke-Json 'POST' "$BaseUrl/api/v1/projects" @{ projectKey = $ProjectKey; name = 'Day5 SDK demo' } 201
...
$null = Invoke-Json 'PUT' "$configPath/evaluation-policy" @{
    ...
    rules = @(@{
        ...
        conditions = @(@{ attribute = 'userId'; operator = 'EQ'; value = 'day5-new' })
    })
}

foreach ($userId in @('day5-old', 'day5-new')) {
    ...
    $expectedFlow = if ($userId -eq 'day5-new') { 'new' } else { 'old' }
...
Write-Output "REAL_MYSQL_DAY5_E2E=PASSED (server must use MySQL); project=$ProjectKey"
